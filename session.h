...
namespace tinymq {
	...
	public:
		...
		void compMapUnlock();
	public:
		std::queue<std::shared_ptr<tinymq::tinyPacket>> _messgaeSendQueue; //待发送的消息队列
		std::set <std::string> topicSubscribed; //此会话对象订阅的话题
		std::map<short, std::shared_ptr<tinymq::tinyPacket>> waitingPubAckMap; //qos1级别消息等待回应
		std::map<short, std::shared_ptr<tinymq::tinyPacket>> waitingPubRecMap; //qos2级别消息等待接受
		std::map<short, std::shared_ptr<tinymq::tinyPacket>> waitingPubCompMap; //qos2级别消息等待完成
